...
class FC_DimRed(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        ####################################
        # plot the result of the t-test between tasks
            
        ttest_res=np.empty(np.shape(self.stats_))
        rows, cols = np.where(self.pvalue_<0.001) # keep only significant values - TODO correction for multiple comparisons
        #print(self.pvalue_[rows, cols])
        
        ttest_res[rows, cols]=self.stats_[rows, cols] 
        max=np.max(np.abs(ttest_res[np.isnan(ttest_res)==0]))
            
        fig_corr = plt.figure(figsize=(21,21), facecolor='white')
        plot_connectivity_circle ( con=ttest_res , node_names=self.chan_names , #node_colors=,
                                                colorbar=True,
                                                colorbar_pos=(0.55, -0.1),
                                                colormap='RdBu_r',
                                                facecolor='white', textcolor='black',
                                                node_edgecolor='white',
                                                title='t-values from '+ self.method + ' (p < 0.001, uncorrected)' ,
                                                fontsize_names=24 ,
                                                fontsize_colorbar=24,
                                                fontsize_title=30,
                                                vmin=-max, vmax=max,
                                                linewidth=5,
                                                fig=fig_corr )
        fig_corr.savefig('Connect_ttest_'+ self.method +'.png', dpi=600, facecolor='white')
